...
# ============================================================================
# We use Qwen2.5-Coder-7B, a 7-billion parameter model optimized for SQL tasks.
# Unsloth provides memory-optimized inference & training for faster GPU training.

max_seq_length = 2048  # Maximum input/output token length
...
trainer = SFTTrainer(
    ...
    args=TrainingArguments(
        # Batch size per GPU (small for 7B model)
        ...
        # Use bf16 if GPU supports it (RTX 4050 does), else fp16
        fp16=not FastLanguageModel.is_bfloat16_supported(),
        bf16=FastLanguageModel.is_bfloat16_supported(),
        logging_steps=1,  # Log metrics every step
        output_dir="outputs",  # Directory to save checkpoints
        report_to="mlflow",  # Send metrics to MLflow for tracking
    ),
)
